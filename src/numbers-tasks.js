...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const anglCos = Math.max(
    -1,
    Math.min(1, dotProduct / (magnitudeVector1 * magnitudeVector2))
  );
  const anlg = Math.acos(anglCos);
  return anlg;
}
...
function getCountOfOddNumbers(number) {
  ...
  return roundNumber % 2 === 0 ? roundNumber / 2 : (roundNumber + 1) / 2;
}
module.exports = {
  getRectangleArea,
